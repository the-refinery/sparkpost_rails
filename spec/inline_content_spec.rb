...
describe SparkPostRails::DeliveryMethod do
  ...
  context 'Inline Content' do
    ...
    it 'does not include template details' do
      test_email = Mailer.test_email
      @delivery_method.deliver!(test_email)

      expect(@delivery_method.data[:content].key?(:template_id)).to eq(false)
      expect(@delivery_method.data[:content].key?(:use_draft_template)).to eq(false)
    end
  end
end
